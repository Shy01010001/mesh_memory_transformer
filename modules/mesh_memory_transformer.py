...
class Transformer(nn.Module):
    def __init__(self, encoder, decoder, src_embed, tgt_embed, cmn = None):
    ...
    def forward(self, src, tgt, src_mask, tgt_mask):
        return self.decode(self.encode(src, src_mask), src_mask, tgt, tgt_mask, memory_matrix=None)
...
class Encoder(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        for i, layer in enumerate(self.layers):
            x = layer(x, mask)
            record.append(x)       
        output = torch.stack(record, dim = 1)
...
class ImageMultiHeadedAttention(nn.Module):
    def __init__(self, h, d_model, dropout=0.1):
        ...
        nn.init.normal_(self.m_k, 0, 1 / (d_model // self.h))
    ...
    def forward(self, query, key, value, mask=None):
        ...
        query, key, value = \
            [l(x) for l, x in zip(self.linears, (query, key, value))]
        Mk = np.sqrt(self.d_model // self.h) * self.m_k.repeat(key.size(0), 1, 1)
...
class MMT(AttModel):
    ...
    def make_model(self, tgt_vocab, cmn = None):
        c = copy.deepcopy
        ...
        model = Transformer(
            Encoder(EncoderLayer(self.d_model, aug_attn, c(ff), self.dropout), self.num_layers),
            Decoder(DecoderLayer(self.d_model, attn, src_attn, c(ff), self.dropout), self.num_layers),
