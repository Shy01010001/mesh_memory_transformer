...
def attention(query, key, value, mask=None, dropout=None, name = None):
    d_k = query.size(-1)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, mask,name = __name__))
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, memory, src_mask, tgt_mask, layer_past=None):
        ...
        if layer_past is None:
            x = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, tgt_mask, name ='decoder_self'))
            x = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask, name = 'decoder_cross'))
            return self.sublayer[2](x, self.feed_forward)
        else:
            present = [None, None]
            x, present[0] = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, tgt_mask, layer_past[0], name = 'decoder_self'))
            x, present[1] = self.sublayer[1](x, lambda x: self.src_attn(x, m, m, src_mask, layer_past[1], name = 'decoder_cross'))
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, layer_past=None, name = None):
        if mask is not None:
            mask = mask.unsqueeze(1)
        nbatches = query.size(0)
        if layer_past is not None and layer_past.shape[2] == key.shape[1] > 1:
        ...
        x, self.attn = attention(query, key, value, mask=mask,
                                 dropout=self.dropout, name = name)
        x = x.transpose(1, 2).contiguous() \
            .view(nbatches, -1, self.h * self.d_k)
        if layer_past is not None:
            return self.linears[-1](x), present
