...
def build_noamopt_optimizer(args, model):
    ...
    ed_optimizer = get_std_opt(model.encoder_decoder, optim_func=args.optim,
                               factor=args.noamopt_factor, warmup=args.noamopt_warmup)
...
class ReduceLROnPlateau(object):
    ...
    def load_state_dict(self, state_dict):
        if 'current_lr' not in state_dict:
            # it's a normal optimizer
            self.optimizer.load_state_dict(state_dict)
            set_lr(self.optimizer, self.current_lr) # use the lr format the option
        else:
            # it's a scheduler
...
def build_plateau_optimizer(args, model):
    ...
    ve_optimizer = ReduceLROnPlateau(ve_optimizer, factor=args.reduce_on_plateau_factor,
    ...
    ed_optimizer = ReduceLROnPlateau(ed_optimizer, factor=args.reduce_on_plateau_factor,
                                     patience=args.reduce_on_plateau_patience)
